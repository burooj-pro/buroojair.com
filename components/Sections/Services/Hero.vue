	...
	<div class="relative grid grid-cols-1 lg:mt-36 lg:grid-cols-5">
				...
				<div class="flex h-full w-full items-center justify-center p-8">
					<div class="text-center uppercase ltr:lg:text-left rtl:lg:text-right">
						<h2 class="text-3xl font-bold text-white lg:text-5xl">
							{{ $t('SERVICES') }}
							<small class="mt-4 block font-medium">{{ $t('THE_EVOLUTION_OF_HIGH_RISE_WINDOW_AND_FACADE_CLEANING_HAS_ARRIVED') }}</small>
						</h2>
					</div>
				</div>
			</div>
		...
		<section class="col-span-2 bg-emerald-600 p-8 py-16 text-base text-white">
			<div class="container mx-auto">
				<p class="text-lg font-semibold">{{ $t('UP_UNTIL_NOW_HIGH_RISE_WINDOW_AND_FACADE_CLEANING') }}</p>
				<p class="mb-12">{{ $t('WITH_THE_INNOVATION_OF_BUROOJ_AIR_WORKING_DRONE') }}</p>
				<p class="text-lg font-semibold">{{ $t('A_MORE_EFFICIENT_SAFER_AND') }}</p>
